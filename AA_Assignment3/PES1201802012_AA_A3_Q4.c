    ...
    while(t--)
    {
        ...
        for(int i=0;i<=m;++i)
        {
            for(int j=0;j<=n;++j)
            {
                ...
                {
                    lpstable[i*(n+1)+j] = (lpstable[i*(n+1)+j-1]+(sn[j-1]-'a'+1))%mod;
                }
                else if(j==0)
                {
                    lpstable[i*(n+1)+j] = (lpstable[(i-1)*(n+1)+j]+(sm[i-1]-'a'+1))%mod;
                }
                else
                    {
                        if (sm[i-1]==sn[j-1])
                            ...
                            lpstable[i*(n+1)+j]=(minfind(lpstable[(i-1)*(n+1)+j-1]+findabs(sm[i-1],sn[j-1]),lpstable[(i-1)*(n+1)+j]+(sm[i-1]-'a'+1),lpstable[(i)*(n+1)+j-1]+(sn[j-1]-'a'+1)))%mod;
                    }             
            }
        }
